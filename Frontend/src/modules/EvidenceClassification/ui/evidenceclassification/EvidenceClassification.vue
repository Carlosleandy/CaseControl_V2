<script setup lang="ts">
    ...
    import type { EvidenceClassification, EvidenceClassificationRow } from '../../domain/model/evidenceclassification'
    import ConfirmPopup from 'primevue/confirmpopup'
    import EvidenceClassificationForm from './Form.vue'
    ...
    const openFormDialog = (record: EvidenceClassification | null, isCloning = false) => {
        recordId.value = record ? record.id : 0;
        showDialog.value = true;
        isCloningRecord.value = isCloning;
    }

    async function deleteEvidenceClassificationById(id: number): Promise<void> {
    ...
    function confirmDelete(event: any, id: number) {
        confirm.require({
            ...
            accept: () => {
            deleteEvidenceClassificationById(id);
            },
        });
    }
        ...
        <template #desktop>
            <div class="card">
                ...
                >
                    <template #header>
                        <div class="flex corn">
                            <h1>{{ $t('Clasificaciones de Evidencias')}}</h1>
