<script setup lang="ts">
    import type { CaseAssignmentFilter } from "../../domain/model/caseassignment"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: CaseAssignmentFilter
	}>();

    
    const filter = ref<CaseAssignmentFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as CaseAssignmentFilter;
