<script setup lang="ts">
    import type { RecommendationStatusFilter } from "../../domain/model/recommendationstatus"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: RecommendationStatusFilter
	}>();

    
    const filter = ref<RecommendationStatusFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as RecommendationStatusFilter;
