<script setup lang="ts">
    ...
    import type { RecommendationType, RecommendationTypeRow } from '../../domain/model/recommendationtype'
    import ConfirmPopup from 'primevue/confirmpopup'
    import RecommendationTypeForm from './Form.vue'
    ...
    const openFormDialog = (record: RecommendationType | null, isCloning = false) => {
        recordId.value = record ? record.id : 0;
        showDialog.value = true;
        isCloningRecord.value = isCloning;
    }

    async function deleteRecommendationTypeById(id: number): Promise<void> {
    ...
    function confirmDelete(event: any, id: number) {
        confirm.require({
            ...
            accept: () => {
            deleteRecommendationTypeById(id);
            },
        });
    }
        ...
        <template #desktop>
            <div class="card">
                ...
                >
                    <template #header>
                        <div class="flex corn">
                            <h1>{{ $t('Tipos de Recomendaciones')}}</h1>
