<script setup lang="ts">
    import type { UserLevelFilter } from "../../domain/model/userlevel"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: UserLevelFilter
	}>();

    
    const filter = ref<UserLevelFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as UserLevelFilter;
