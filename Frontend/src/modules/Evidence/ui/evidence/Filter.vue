<script setup lang="ts">
    import type { EvidenceFilter } from "../../domain/model/evidence"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: EvidenceFilter
	}>();

    
    const filter = ref<EvidenceFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as EvidenceFilter;
