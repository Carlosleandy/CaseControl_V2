<script setup lang="ts">
    import type { ReceptionMediumFilter } from "../../domain/model/receptionmedium"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: ReceptionMediumFilter
	}>();

    
    const filter = ref<ReceptionMediumFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as ReceptionMediumFilter;
