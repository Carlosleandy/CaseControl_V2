<script setup lang="ts">
    import type { InterviewFilter } from "../../domain/model/interview"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: InterviewFilter
	}>();

    
    const filter = ref<InterviewFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as InterviewFilter;
