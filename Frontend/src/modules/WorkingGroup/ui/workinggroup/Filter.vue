<script setup lang="ts">
    import type { WorkingGroupFilter } from "../../domain/model/workinggroup"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: WorkingGroupFilter
	}>();

    
    const filter = ref<WorkingGroupFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as WorkingGroupFilter;
