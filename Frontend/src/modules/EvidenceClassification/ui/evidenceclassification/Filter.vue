<script setup lang="ts">
    import type { EvidenceClassificationFilter } from "../../domain/model/evidenceclassification"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: EvidenceClassificationFilter
	}>();

    
    const filter = ref<EvidenceClassificationFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as EvidenceClassificationFilter;
