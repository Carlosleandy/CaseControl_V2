<script setup lang="ts">
    ...
    import type { CaseType, CaseTypeRow } from '../../domain/model/casetype'
    import ConfirmPopup from 'primevue/confirmpopup'
    import CaseTypeForm from './Form.vue'
    ...
    const openFormDialog = (record: CaseType | null, isCloning = false) => {
        recordId.value = record ? record.id : 0;
        showDialog.value = true;
        isCloningRecord.value = isCloning;
    }
    
async function deleteCaseTypeById(id: number): Promise<void> {
        ...
        <template #desktop>
            <div class="card">
                ...
                >
                    <template #header>
                        <div class="flex corn">
                            <h1>{{ $t('Tipos de Casos')}}</h1>
    
                            <div style="min-width: 40%;">
                                <InputSearch v-model="filter.filter" @onChange="getAllRecords(search)" />
                            </div>
                        </div>
                    </template>
                    ...
                    <!-- <template #footer>
                        <PaginationInfo :shown="entities.length" :total="totalRecords"></PaginationInfo>                        
                    </template>               -->
                </DataTable>     
