<script setup lang="ts">
    import type { FaultLinkedFilter } from "../../domain/model/faultlinked"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: FaultLinkedFilter
	}>();

    
    const filter = ref<FaultLinkedFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as FaultLinkedFilter;
