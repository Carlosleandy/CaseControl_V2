<script setup lang="ts">
    import type { LinkTypeFilter } from "../../domain/model/linktype"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: LinkTypeFilter
	}>();

    
    const filter = ref<LinkTypeFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as LinkTypeFilter;
