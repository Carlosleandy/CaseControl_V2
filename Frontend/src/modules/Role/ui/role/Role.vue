<script setup lang="ts">
    ...
    import type { Role, RoleRow } from '../../domain/model/role'
    ...
    const { roleList, getAllRoles, RemoveRow } = useDataRole();
    ...
    const openFormDialog = (record: Role | null, isCloning = false) => {
        recordId.value = record ? record.id : 0;
        showDialog.value = true;
        isCloningRecord.value = isCloning;
    }

    async function deleteRoleById(id: number): Promise<void> {
                    ...
                    <template #header>
                        <div class="flex corn">
                            ...
                            <div style="min-width: 40%;">
                                <!-- <InputSearch v-model="filter.filter" @onChange="getAllRoles()" /> -->
                            </div>
                        </div>
                    </template>
