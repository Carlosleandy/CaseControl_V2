<script setup lang="ts">
    ...
    import type { RecommendationStatus, RecommendationStatusRow } from '../../domain/model/recommendationstatus'
    import ConfirmPopup from 'primevue/confirmpopup'
    import RecommendationStatusForm from './Form.vue'
    ...
    const openFormDialog = (record: RecommendationStatus | null, isCloning = false) => {
        recordId.value = record ? record.id : 0;
        showDialog.value = true;
        isCloningRecord.value = isCloning;
    }

    async function deleteRecommendationStatusById(id: number): Promise<void> {
    ...
    function confirmDelete(event: any, id: number) {
        confirm.require({
            ...
            accept: () => {
            deleteRecommendationStatusById(id);
            },
        });
    }
        ...
        <template #desktop>
            <div class="card">
                ...
                >
                    <template #header>
                        <div class="flex corn">
                            <h1>{{ $t('Estados de Recomendaciones')}}</h1>
