<script setup lang="ts">
    import type { CaseStatusFilter } from "../../domain/model/casestatus"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: CaseStatusFilter
	}>();

    
    const filter = ref<CaseStatusFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as CaseStatusFilter;
