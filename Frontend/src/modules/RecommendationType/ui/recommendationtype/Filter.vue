<script setup lang="ts">
    import type { RecommendationTypeFilter } from "../../domain/model/recommendationtype"
    import { ref } from 'vue'
    
    const emit = defineEmits(['update:modelValue', 'onChage']);
    
    const props = defineProps<{
		modelValue: RecommendationTypeFilter
	}>();

    
    const filter = ref<RecommendationTypeFilter>({...props.modelValue});

    const clearFilter = () => {
        filter.value = {} as RecommendationTypeFilter;
